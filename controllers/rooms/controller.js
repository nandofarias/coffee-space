...
async function sendMessage(socket, data) {
  try {
    ...
    if (!room) io.in(socket).emit('client_error', { message: 'Room not found' });
...
async function createRoom(socket, data) {
  try {
    ...
    const destination = { type: data.destination.userType, referenceId: data.destination.userId };
    const room = await Room.findOrCreate([user, destination]);
